...
class Spinner:
    ...
    def spin(self):
        while self.running:
            ...
            print(f"\r{Colors.CYAN}{frame}{Colors.END} {self.message}... - telegram_bot.py:92", end="", flush=True)
            self.idx += 1
            time.sleep(0.1)
    ...
    def stop(self, final_message: str = None):
        ...
        if final_message:
            print(f"\r{Colors.GREEN}✓{Colors.END} {final_message} - telegram_bot.py:106" + " " * 20)
        else:
            print("\r - telegram_bot.py:108" + " " * 50 + "\r", end="")
...
def print_section(title: str, icon: str = "📌"):
    ...
    line = "─" * (50 - len(title))
    print(f"\n{Colors.CYAN}{icon} {Colors.BOLD}{title}{Colors.END} {Colors.DIM}{line}{Colors.END} - telegram_bot.py:134")
...
def print_status(message: str, status: str = "info"):
    ...
    print(f"{icon} {message} - telegram_bot.py:151")
...
# Real MCP host (Option A): Ollama + qwen3:8b driving true MCP stdio servers.
...
class TelegramBot:
    ...
    def __init__(self, bot_token: str = None):
        ...
        self.commands = {
            ...
            # Real MCP host (Option A): qwen3:8b + true MCP stdio tools
            "/mcp": self.cmd_mcp,
            "/m": self.cmd_mcp,
        ...
        # Real MCP host (Option A): qwen3:8b driving true MCP stdio servers.
        ...
        if MCP_HOST_AVAILABLE:
            try:
                self.mcp_runner = MCPRunner(model="qwen3:8b")
...
/mcp       • Real MCP host (qwen3:8b + tools) 🧰
...
✅ Bot is running"""
    ...
    def _write_live_status(self):
        ...
        try:
            ...
            status_file = Path(__file__).parent / "logs" / "telegram_live_status.json"
    ...
    def cmd_health(self, chat_id: int, args: str) -> str:
        ...
        if self.mcp_runner:
            parts.append(f"• MCP Host (qwen3:8b): {self.mcp_runner.status_line()}")
        else:
            parts.append("• MCP Host (qwen3:8b): ❌")
    ...
    def cmd_mcp(self, chat_id: int, args: str) -> str:
        """Run a prompt through the real MCP host (qwen3:8b decides which
        ...
        if not args or not args.strip():
            return ("Usage: /mcp <prompt>\n"
                    "Runs your prompt through qwen3:8b with real MCP tools "
        ...
        self._write_live_status()
        return f"🧰 MCP host working on: {prompt[:80]}…\n(qwen3:8b is choosing tools — reply will follow)"
